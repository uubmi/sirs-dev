...
function runCheck (oberservationResultsArray) {
	...
	console.log(" "+missingData.nMetCriteria+" what is there "+missingData.metObs.length+"");
	...
	if (missingData.nMetCriteria == 0) { //if nothing is missing just ask for SIRS criteria check and send result
		...
		function buildSIRScriteriaMessage (oberservationResultsArray){
			...
			console.log("hellow?"+knowledgeExecutionEngineResult.nMetCriteria+" ");
		...
		console.log("  !! where data !!" );
			...
			//look to which criteria is missing completely
			...
			for (var i = 0, obs; obs = missingData.metObs[i++]; ) {
				...
				for (var j = 0, obsToCheck; obsToCheck = oberservationResultsArray[j++];) {
					//obsToCheck = oberservationResultsArray[j];
					console.log("length "+i+" "+j+" "+oberservationResultsArray.length+" in the box ");//+oberservationResultsArray[j]+"");
				console.log("check "+obs["observationFocus"]["code"]+" and "+obsToCheck["observationFocus"]["code"]);
					if (obs["observationFocus"]["code"] == oberservationResultsArray[j-1]["observationFocus"]["code"]) {
						//replace with dummy nonRiskValue
						//con
						var code = oberservationResultsArray[j-1]["observationFocus"]["code"];
						oberservationResultsArray[j-1] = nonRiskValues[code];
console.log("  !! new observation!! "+oberservationResultsArray[j-1]["observationFocus"]["displayName"]+": "+oberservationResultsArray[j-1]["observationValue"]["value"] );
					}
					else {
			...
			return oberservationResultsArray;
		}
		...
		var missingSirsCriteria = {assessmentPlan :
			{minRequirement: 1, 
				...
				{minRequirement: 1, 
					...
					new Rule("2.16.840.1.113883.6.5", "386725007", "doesNotExist", "decimal", 0)  // PaCO2 < 32(mmHg) must exist
					)
				},
				{minRequirement: 1, 
					rules : new Array (   
...
function assessMissing(obsResults, assessmentPlan, level) {
 ...
 //                   console.log(" criteria met "+rule["code"]+" "+rule["comparison"]+" "+rule["value"]+"("+obs["observationValue"]["value"]+")");
			...
			if (numNonApplicableCriteria == obsResults.length) { //then the criteria for the Rule is not Present at all!
				...
				metObsResults.push(subsitituteForMissing);
				completelyMissing.push(subsitituteForMissing );
			}
...
//        console.log("level "+level+", criteria met: "+criteriaMet+" minReq: " + assessmentPlan["minRequirement"]);
        return { 
			...
			completelyMissing: completelyMissing 
        };
    } else {
//        console.log("level "+level+", criteria met: "+criteriaMet+" minReq: " + assessmentPlan["minRequirement"]);
        return { 
